...
class EventLogFlushSchedulerTest {
    ...
    /*
        flushApp에서 Redis 버퍼에 항목이 없으면 앱 등록 해제만 수행하는지 검증
     */
    @Test
    void flushApp_when_no_items_unregisters_app_and_does_nothing_else() {
        Long appId = 10L;
        String key = "event-log:buffer:10";
        when(store.bufferKey(appId)).thenReturn(key);
        ...
        verify(store, times(1)).unregisterApp(appId);
    ...
    void flushApp_when_saveAll_fails_and_some_save_fails_still_continues_and_trims() throws Exception {
        ...
        verify(listOps, times(1)).trim(key, 2, -1);
        verify(store, times(1)).pushDeadLetter(eq(appId), anyString(), startsWith("DB_SAVE_FAIL:"));
    }
